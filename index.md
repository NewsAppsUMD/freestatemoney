...
fetches bulk data directly from the SBE’s CRIS API — the same data the
download page at
...
Full-cycle transaction files run to hundreds of megabytes. The
...
- **Direct downloads**:
  [`md_download()`](https://newsappsumd.github.io/freestatemoney/reference/md_download.md)
  pulls bulk data straight from the CRIS API
...
- **Wrong-file detection**: Passing a contributions file to
  ...
  produces a helpful error instead of a broken data frame
